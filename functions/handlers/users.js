...
exports.addUserDetails = (req,res) =>{
  ...
  .then (()=>{
    return res.json({message: 'details added done'});
  })
  .catch(err=>{
    console.error(err);
  ...
  db.doc(`/users/${req.params.handle}`)
    ...
    .then((doc)=>{
      if(doc.exists){
        ...
        return db.collection('problems').where('userHandle','==',req.params.handle)
...
//image upload
 exports.uploadImage =( req,res) => {
    ...
    busboy.on('finish',()=>{
      ...
      .then(() => {
        return res.json({message: 'image upload succeess'});
      })
      .catch(err =>{
        console.error(err);
        return res.status(500).json({error: err.code});
